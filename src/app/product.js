...
import { productCard, productList } from "../core/selector"
...
export const createProductCard=({id,title,description,image,price,rating:{rate,count}})=>{
    ...
    const productCardPrice=productCardUi.querySelector(".product-cart-price .price")
    ...
    const ratingStars=productCardUi.querySelector(".rating-stars")
    ...
    ratingStars.innerHTML=stars(rate)
...
export const stars=(rate)=>{
    ...
    for(let i=1;i<=5;i++){
        starUi+=`<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 stroke-orange-400 ${i<=rate.toFixed(0) && 'fill-orange-600'}">
